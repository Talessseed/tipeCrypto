#include "stdafx.h"
#include "stdio.h"
#include <iostream>
#include <string>
#include <fstream>
#include <sstream>
#include <vector>

#include "windows.h"
using namespace std;

typedef unsigned char byte;


byte s_box[256] = // c'est sur wikipédia
{
	
};

byte s_box_inv[256] =
{
	
};

byte mul2[] = {
	
};

byte mul3[] = {
	
};
 
byte mul9[] = {
	
};

byte mul11[] = {
	
};

byte mul13[] = {
	
};

byte mul14[] = {
	
};

byte rcon[256] = {
	
};


/* AUCUN BUG */
void rotate(byte* word)
{
	byte tmp;
	tmp = word[0];

	for (unsigned int i = 0; i < 3; i++)
		word[i] = word[i + 1];

	word[3] = tmp;
}

/* AUCUN BUG */
void schedule(byte *word, unsigned int i)
{
	rotate(word);

	for (unsigned int i = 0; i < 4; i++)
		word[i] = s_box[word[i]];

	word[0] ^= rcon[i];
}

/* AUCUN BUG (pour 128 bit au moins) */
void _KeyExpansion_(byte* state, byte* turn_keys, unsigned int n, unsigned int b, bool is256)
{

	for (unsigned int i = 0; i < n; i++)
	{
		turn_keys[i] = state[i];
	}

	byte t[4];
	byte c = n;
	byte i = 1;

	while (c < b) {

		for (byte j = 0; j < 4; j++)
			t[j] = turn_keys[j + c - 4];

		if (c % n == 0) {
			schedule(t, i);
			i++;
		}

		if (is256 && c % 32 == 16) {
			for (byte j = 0; j < 4; j++)
				t[j] = s_box[t[j]];
		}

		for (byte j = 0; j < 4; j++) {
			turn_keys[c] = turn_keys[c - n] ^ t[j];
			c++;
		}
	}

}

/* AUCUN BUG (pour 128 bit au moins) */
void KeyExpansion(byte* state, byte* turn_keys, unsigned int KEY_SIZE) // pk pas un truc avec juste 2 variables : 
{                                                                      // inputKey[16] et expandedKeys[176]
																	   // -> parce que si on change la taille de 
																	   // la clef en 256 bits ca marche plus
	switch (KEY_SIZE)
	{
	case 128:
		_KeyExpansion_(state, turn_keys, 16, 176, false);
		break;
	case 192:
		_KeyExpansion_(state, turn_keys, 24, 208, false);
		break;
	case 256:
		_KeyExpansion_(state, turn_keys, 32, 240, true);
		break;
	default:
		printf("! Unknown KEY_SIZE !\n");
	}
}

void SubBytes(byte* state)
{
	for (unsigned int i = 0; i < 16; i++)
		state[i] = s_box[state[i]];
}

void SubBytes_inv(byte* state)
{
	for (unsigned int i = 0; i < 16; i++)
		state[i] = s_box_inv[state[i]];
}

/* AUCUN BUG */
void ShiftRows(byte* state)
{
	byte tmp[16]; //on fait le shifting la-dedans

	tmp[0] = state[0];
	tmp[1] = state[5];
	tmp[2] = state[10];
	tmp[3] = state[15];

	tmp[4] = state[4];
	tmp[5] = state[9];
	tmp[6] = state[14];
	tmp[7] = state[3];

	tmp[8] = state[8];
	tmp[9] = state[13];
	tmp[10] = state[2];
	tmp[11] = state[7];

	tmp[12] = state[12];
	tmp[13] = state[1];
	tmp[14] = state[6];
	tmp[15] = state[11];

	for (int i = 0; i < 16; i++)
		state[i] = tmp[i];
}

void ShiftRows_inv(byte* state)
{
	byte tmp[16]; //on fait le shifting la-dedans

	for (int i = 0; i < 16; i++)
		tmp[i] = state[i];

	state[0] = tmp[0];
	state[1] = tmp[5];
	state[2] = tmp[10];
	state[3] = tmp[15];

	state[4] = tmp[4];
	state[5] = tmp[9];
	state[6] = tmp[14];
	state[7] = tmp[3];

	state[8] = tmp[8];
	state[9] = tmp[13];
	state[10] = tmp[2];
	state[11] = tmp[7];

	state[12] = tmp[12];
	state[13] = tmp[1];
	state[14] = tmp[6];
	state[15] = tmp[11];
}


void MixColumns(byte* state)
{
	byte tmp[16];

	tmp[0] = (byte)(mul2[state[0]] ^ mul3[state[1]] ^ state[2] ^ state[3]);
	tmp[1] = (byte)(state[0] ^ mul2[state[1]] ^ mul3[state[2]] ^ state[3]);
	tmp[2] = (byte)(state[0] ^ state[1] ^ mul2[state[2]] ^ mul3[state[3]]);
	tmp[3] = (byte)(mul3[state[0]] ^ state[1] ^ state[2] ^ mul2[state[3]]);

	tmp[4] = (byte)(mul2[state[4]] ^ mul3[state[5]] ^ state[6] ^ state[7]);
	tmp[5] = (byte)(state[4] ^ mul2[state[5]] ^ mul3[state[6]] ^ state[7]);
	tmp[6] = (byte)(state[4] ^ state[5] ^ mul2[state[6]] ^ mul3[state[7]]);
	tmp[7] = (byte)(mul3[state[4]] ^ state[5] ^ state[6] ^ mul2[state[7]]);

	tmp[8] = (byte)(mul2[state[8]] ^ mul3[state[9]] ^ state[10] ^ state[11]);
	tmp[9] = (byte)(state[8] ^ mul2[state[9]] ^ mul3[state[10]] ^ state[11]);
	tmp[10] = (byte)(state[8] ^ state[9] ^ mul2[state[10]] ^ mul3[state[11]]);
	tmp[11] = (byte)(mul3[state[8]] ^ state[9] ^ state[10] ^ mul2[state[11]]);

	tmp[12] = (byte)(mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^ state[15]);
	tmp[13] = (byte)(state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^ state[15]);
	tmp[14] = (byte)(state[12] ^ state[13] ^ mul2[state[14]] ^ mul3[state[15]]);
	tmp[15] = (byte)(mul3[state[12]] ^ state[13] ^ state[14] ^ mul2[state[15]]);

	for (int i = 0; i < 16; i++)
		state[i] = tmp[i];
}

void MixColumns_inv(byte* state)
{
	byte tmp[16];

	tmp[0] = (byte)(mul14[state[0]] ^ mul11[state[1]] ^ mul13[state[2]] ^ mul9[state[3]]);
	tmp[1] = (byte)(mul9[state[0]] ^ mul14[state[1]] ^ mul11[state[2]] ^ mul13[state[3]]);
	tmp[2] = (byte)(mul13[state[0]] ^ mul9[state[1]] ^ mul14[state[2]] ^ mul11[state[3]]);
	tmp[3] = (byte)(mul11[state[0]] ^ mul13[state[1]] ^ mul9[state[2]] ^ mul14[state[3]]);

	tmp[4] = (byte)(mul14[state[4]] ^ mul11[state[5]] ^ mul13[state[6]] ^ mul9[state[7]]);
	tmp[5] = (byte)(mul9[state[4]] ^ mul14[state[5]] ^ mul11[state[6]] ^ mul13[state[7]]);
	tmp[6] = (byte)(mul13[state[4]] ^ mul9[state[5]] ^ mul14[state[6]] ^ mul11[state[7]]);
	tmp[7] = (byte)(mul11[state[4]] ^ mul13[state[5]] ^ mul9[state[6]] ^ mul14[state[7]]);

	tmp[8] = (byte)(mul14[state[8]] ^ mul11[state[9]] ^ mul13[state[10]] ^ mul9[state[11]]);
	tmp[9] = (byte)(mul9[state[8]] ^ mul14[state[9]] ^ mul11[state[10]] ^ mul13[state[11]]);
	tmp[10] = (byte)(mul13[state[8]] ^ mul9[state[9]] ^ mul14[state[10]] ^ mul11[state[11]]);
	tmp[11] = (byte)(mul11[state[8]] ^ mul13[state[9]] ^ mul9[state[10]] ^ mul14[state[11]]);

	tmp[12] = (byte)(mul14[state[12]] ^ mul11[state[13]] ^ mul13[state[14]] ^ mul9[state[15]]);
	tmp[13] = (byte)(mul9[state[12]] ^ mul14[state[13]] ^ mul11[state[14]] ^ mul13[state[15]]);
	tmp[14] = (byte)(mul13[state[12]] ^ mul9[state[13]] ^ mul14[state[14]] ^ mul11[state[15]]);
	tmp[15] = (byte)(mul11[state[12]] ^ mul13[state[13]] ^ mul9[state[14]] ^ mul14[state[15]]);

	for (int i = 0; i < 16; i++)
		state[i] = tmp[i];
}


/*
BUGGE, PROBABLEMENT LA MULTIPLICATION PAR LA MATRICE
void _MixColumns_(byte* mat, byte* state)
{
for (unsigned int i = 0; i < 4; i++)
{
byte column[4] = { state[i], state[4 + i], state[8 + i], state[12 + i] };
mulmat4(mat, column);
state[i] = column[0];
state[4 + i] = column[1];
state[8 + i] = column[2];
state[12 + i] = column[3];
}
}

void MixColumns(byte* state)
{
_MixColumns_(MixColumsMat, state);
}

void MixColumns_inv(byte* state)
{
_MixColumns_(MixColumsMat_inv, state);
}*/

void AddRoundKey(byte* state, byte* roundKey)
{
	for (int i = 0; i < 16; i++)
		state[i] ^= roundKey[i]; // le XOR
}

void AES_Decrypt(byte* message, byte* turn_keys, byte* state, unsigned int KEY_SIZE)
{
	for (int i = 0; i < 16; i++)
		state[i] = message[i];

	unsigned int numberOfRounds = 0;

	switch (KEY_SIZE)
	{
	case 128:
		numberOfRounds = 10;
		break;
	case 192:
		numberOfRounds = 12;
		break;
	case 256:
		numberOfRounds = 14;
		break;
	default:
		printf("! Unknown KEY_SIZE !\n");
	}

	byte tkey[16];

	for (unsigned int j = KEY_SIZE - 16; j < KEY_SIZE; j++)
		tkey[j] = turn_keys[j];

	AddRoundKey(state, tkey); // Whitening/AddRoundKey
	ShiftRows_inv(state);
	SubBytes_inv(state);


	for (int i = numberOfRounds - 1; i >= 0; i--)
	{
		for (unsigned int j = 0; j < 16; j++)
			tkey[j] = turn_keys[16 * i + j];

		AddRoundKey(state, tkey);
		MixColumns_inv(state);
		ShiftRows_inv(state);
		SubBytes_inv(state);
	}

	for (unsigned int j = 0; j < 16; j++)
		tkey[j] = turn_keys[j];

	AddRoundKey(state, tkey); // Whitening/AddRoundKey

}

void AES_Encrypt(byte* message, byte* turn_keys, byte* state, unsigned int KEY_SIZE)
{
	for (int i = 0; i < 16; i++)
		state[i] = message[i];

	unsigned int numberOfRounds = 0;

	switch (KEY_SIZE)
	{
	case 128:
		numberOfRounds = 10;
		break;
	case 192:
		numberOfRounds = 12;
		break;
	case 256:
		numberOfRounds = 14;
		break;
	default:
		printf("! Unknown KEY_SIZE !\n");
	}

	byte tkey[16];



	for (unsigned int j = 0; j < 16; j++)
		tkey[j] = turn_keys[j];

	AddRoundKey(state, tkey); // Whitening/AddRoundKey


	for (unsigned int i = 1; i < numberOfRounds; i++)
	{
		for (unsigned int j = 0; j < 16; j++)
			tkey[j] = turn_keys[16 * i + j];

		SubBytes(state);
		ShiftRows(state);
		MixColumns(state);
		AddRoundKey(state, tkey);
	}

	for (unsigned int j = 0; j < 16; j++)
		tkey[j] = turn_keys[16 * numberOfRounds + j];

	// Final round
	SubBytes(state);
	ShiftRows(state);
	AddRoundKey(state, tkey);
}

int chex_to_int(char input)
{
	if (input >= '0' && input <= '9')
		return input - '0';
	if (input >= 'A' && input <= 'F')
		return input - 'A' + 10;
	if (input >= 'a' && input <= 'f')
		return input - 'a' + 10;
	throw std::invalid_argument("Invalid input string");
}

int main()
{
	std::string tmpStr;

	unsigned int numberOfRounds = 0;

	/* KEY INSERTION */

	while (numberOfRounds == 0)
	{
		printf("Enter key : ");
		std::getline(std::cin, tmpStr);

		switch (tmpStr.length()*8)
		{
		case 128:
			numberOfRounds = 10;
			break;
		case 192:
			numberOfRounds = 12;
			break;
		case 256:
			numberOfRounds = 14;
			break;
		default:
			printf("! Unknown key size (must be 128/192/256 bits) !\n");
		}
	}
	
	unsigned int KEY_SIZE = tmpStr.length() * 8;
	printf("Key size : %u\n", KEY_SIZE);

	byte* key = new byte[tmpStr.length() + 1];
	strcpy((char*)key, tmpStr.c_str());

	/* MESSAGE INSERTION */

	char cryptC;
	printf("Encrypt ? (y/n) : ");
	cin >> cryptC;

	bool crypt = cryptC == 'y';

	cin.clear();
	cin.ignore(10000, '\n');

	tmpStr = "";

	if (crypt)
	{
		printf("Enter message :\n\n");
		std::getline(std::cin, tmpStr);
	}
	else
	{
		std::string tmpHexStr;

		printf("Enter message :\n\n");
		std::getline(std::cin, tmpHexStr);

		/* HEX MESSAGE TO BYTE ARRAY */
		
		std::string buf;
		std::stringstream ss(tmpHexStr);

		while (ss >> buf)		// Iterate words
		{
			byte b = 0;
			if (buf.length() == 1)
				b = chex_to_int(buf[0]);
			else
				b = chex_to_int(buf[0]) * 16 + chex_to_int(buf[1]);

			tmpStr += (const char)b;
		}
	}
	
	byte* message = new byte[tmpStr.length() + 1];
	strcpy((char*)message, tmpStr.c_str());

	/* TURN KEYS GENERATION */

	byte* turn_keys = new byte[(numberOfRounds + 1) * 16];

	KeyExpansion(key, turn_keys, KEY_SIZE);

	/* MESSAGE PADDING */

	// longueur du msg ajustée
	unsigned int originalLen = strlen((const char*)message);
	unsigned int lenOfPaddedMessage = originalLen;

	if (lenOfPaddedMessage % 16 != 0)
		lenOfPaddedMessage = (lenOfPaddedMessage / 16 + 1) * 16;

	byte* paddedMessage = new byte[lenOfPaddedMessage];
	byte* newMessage = new byte[lenOfPaddedMessage];


	for (unsigned int i = 0; i < originalLen; i++)
		paddedMessage[i] = message[i];

	for (unsigned int i = originalLen; i < lenOfPaddedMessage; i++)
		paddedMessage[i] = 0;

	/* ENCRYPTION */

	byte* tmp = new byte[16];

	if (crypt)
	{
		// Encrypt padded message :
		for (unsigned int i = 0; i < lenOfPaddedMessage; i += 16)
		{
			AES_Encrypt(paddedMessage + i, turn_keys, tmp, KEY_SIZE);
			for (unsigned int j = 0; j < 16; j++)
			{
				newMessage[i + j] = tmp[j];
			}
			printf("-");
		}
		printf("\n");

		/* ENCRYPTED MESSAGE DISPLAY */

		for (unsigned int j = 0; j < lenOfPaddedMessage; j++)
		{
			printf("%X ", newMessage[j]);
			if (j % 16 == 15)
				printf("\n");
		}
	}
	else
	{
		// Decrypt padded message :
		for (unsigned int i = 0; i < lenOfPaddedMessage; i += 16)
		{
			AES_Decrypt(paddedMessage + i, turn_keys, tmp, KEY_SIZE);
			for (unsigned int j = 0; j < 16; j++)
			{
				newMessage[i + j] = tmp[j];
			}
			printf("-");
		}
		printf("\n");

		/* DECRYPTED MESSAGE DISPLAY */

		printf((const char*)newMessage);
	}
	

	

	char outputToFile;

	/* WRITE TO FILE */

	printf("Output to file (y/n) : ");
	cin >> outputToFile;

	if (outputToFile == 'y')
	{
		printf("File name : ");
		std::cin >> tmpStr;

		ofstream myfile;
		myfile.open(tmpStr + ".txt");
		std::cout << "Writing to file : " << tmpStr << ".txt";

		for (unsigned int j = 0; j < lenOfPaddedMessage; j++)
		{
			if (crypt)
			{
				myfile << std::hex << (unsigned int)newMessage[j] << " ";
			}
			else
			{
				myfile << newMessage[j];
			}
			
		}
		myfile.close();
	}

	delete message;
	delete key;
	delete newMessage;
	delete paddedMessage;
	delete turn_keys;
}
